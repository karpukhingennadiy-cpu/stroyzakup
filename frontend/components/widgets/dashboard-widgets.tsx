...
/* ------------------------------------------------------------------ */
/* Общая обёртка виджета (карточка)                                    */
/* ------------------------------------------------------------------ */

function WidgetCard({ icon, title, meta, children }: { icon: React.ReactNode; title: string; meta?: string; children: React.ReactNode }) {
  return (
    <section className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--separator)] bg-[var(--bg-tertiary)] shadow-[var(--shadow-xs)]" aria-label={title}>
      <header className="px-4 py-3 border-b border-[var(--separator)] bg-[var(--fill-1)] flex items-center gap-2">
        <span className="text-[var(--label-tertiary)]">{icon}</span>
        <h2 className="text-sm font-medium text-[var(--label-primary)]">{title}</h2>
        {meta && <span className="ml-auto text-xs text-[var(--label-tertiary)]">{meta}</span>}
      </header>
...
const STATUS_GROUPS = [
  {
    key: "total", label: "Всего", tone: "text-[var(--label-primary)]",
    match: () => true,
  },
  {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3" role="group" aria-label="Статистика заявок">
      {STATUS_GROUPS.map((g) => {
        ...
        return (
          <div key={g.key} className="rounded-[var(--radius-lg)] border border-[var(--separator)] bg-[var(--bg-tertiary)] px-4 py-4 shadow-[var(--shadow-xs)]">
            <p className="text-xs text-[var(--label-tertiary)]">{g.label}</p>
            <p className={"mt-1 text-2xl font-semibold tracking-tight leading-8 tabular-nums " + g.tone} aria-label={g.label + ": " + count}>
              {count}
            </p>
          </div>
        );
...
export function SuppliersMapWidget({ lat, lon }: { lat: number; lon: number }) {
  ...
  return (
    ...
    >
      {loading ? (
        <p className="text-sm text-[var(--label-tertiary)] py-10 text-center" role="status">Загрузка карты...</p>
      ) : failed ? (
        <p className="text-sm text-[var(--label-tertiary)] py-10 text-center">Не удалось загрузить поставщиков</p>
      ) : (
        <svg viewBox="0 0 300 300" className="w-full h-auto" role="img" aria-label={"Поставщики на карте: " + points.length}>
          ...
          <circle cx="150" cy="150" r="5" fill="var(--brand)" stroke="var(--bg-tertiary)" strokeWidth="1.5">
            <title>Точка доставки</title>
          </circle>
        </svg>
      )}
      <p className="mt-2 text-xs text-[var(--label-tertiary)] text-center">
...
export function PriceChartWidget({ requestId }: { requestId: number | null }) {
  ...
  return (
    <WidgetCard
      ...
      title="График цен"
      meta="по конкурентному листу"
    >
      {loading ? (
        <p className="text-sm text-[var(--label-tertiary)] py-10 text-center" role="status">Загрузка цен...</p>
      ) : failed || !requestId || rows.length === 0 ? (
        <div className="py-10 text-center">
          <IconTruck className="w-8 h-8 mx-auto text-[var(--label-quaternary)]" />
          <p className="mt-2 text-sm text-[var(--label-tertiary)]">
            ...
            return (
              <li key={s.supplier_id ?? s.supplier_name}>
                <div className="flex items-baseline justify-between gap-2 mb-1">
                  <span className="text-xs text-[var(--label-secondary)] truncate">
                    {s.supplier_name}
                    {isBest && <span className="ml-1.5 text-[var(--success)] font-medium">лучшее</span>}
                  </span>
                  <span className="text-xs font-medium text-[var(--label-primary)] shrink-0 tabular-nums">
